...
pub async fn run(clients: Clients) {
    ...
    tokio::spawn(gpio_sender(input_sender, 21)); // pin 40 = GPIO 21
...
async fn gpio_sender(sender: mpsc::UnboundedSender<Sample>, line: u32) -> Result<()> {
    ...
    let mut events = AsyncLineEventHandle::new(
        input.events(LineRequestFlags::INPUT, EventRequestFlags::RISING_EDGE, "read-gpio")?
    )?;
    ...
    loop {
        match events.next().await {
            Some(Ok(evt)) => {
                ...
                sender.send((evt.timestamp() + 500) / 1000)?; // round half up ns to ms
            }
            Some(Err(e)) => {
                eprintln!("Error: {}", e);
            }
